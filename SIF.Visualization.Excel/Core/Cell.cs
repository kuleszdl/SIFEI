...
namespace SIF.Visualization.Excel.Core
{
    ...
    public class Cell : BindableBase
    {
        ...
        public bool IsSelected
        {
            ...
            {
                isSelected = value;
                foreach (var vio in Violations) vio.IsCellSelected = value;
        ...
        {
            get { return ruleCellType; }
            set { SetProperty(ref ruleCellType, value); }
        }
        ...
        private void extractLocation(string address)
        {
            ...
            var worksheet = (from Worksheet p in Workbook.Worksheets where p.Name == worksheetKey select p)
                .FirstOrDefault();
            ...
            var shortAddress = address.Substring(address.IndexOf('!') + 1);
        ...
        public void RecalculateVisibleViolations()
        {
            ...
            if (VisibleViolations.Count > 0)
            {
                ...
                ViolationsPane.SortDescriptions.Add(
                    new SortDescription("FirstOccurrence", ListSortDirection.Descending));
        ...
        public void SetVisibility()
        {
            ...
            if (control != null)
            {
                foreach (var violation in violations)
                    if (violation.ViolationState.Equals(ViolationState.OPEN) &&
                        tab.Equals(WorkbookModel.Tabs.OpenViolations))
                    {
                        control.Visible = true;
                        return;
                    }
                    else if (violation.ViolationState.Equals(ViolationState.LATER) &&
                             tab.Equals(WorkbookModel.Tabs.LaterViolations))
                    {
                        control.Visible = true;
                        return;
                    }
                    else if (violation.ViolationState.Equals(ViolationState.IGNORE) &&
                             tab.Equals(WorkbookModel.Tabs.IgnoredViolations))
                    {
                        control.Visible = true;
                        return;
                    }
                    else if (violation.ViolationState.Equals(ViolationState.SOLVED) &&
                             tab.Equals(WorkbookModel.Tabs.ArchivedViolations))
                    {
                        control.Visible = true;
                        return;
                    }
