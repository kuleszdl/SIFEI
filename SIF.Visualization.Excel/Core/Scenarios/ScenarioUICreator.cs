...
namespace SIF.Visualization.Excel.Core.Scenarios
{
    public class ScenarioUICreator
    {
        ...
        private readonly List<ScenarioDataFieldContainer> containers = new List<ScenarioDataFieldContainer>();
        private Workbook workbook;
        private Scenario newScenario;
        private static readonly object syncScenario = new object();
        ...
        public void Start(WorkbookModel wb, string scenarioTitle)
        {
            ...
            workingList.Sort(delegate(Cell x, Cell y)
            {
                ...
                if (xSheet.Index < ySheet.Index) return -1;
                if (xSheet.Index > ySheet.Index) return 1;
                //sort by column
                var xRange = xSheet.Range[x.ShortLocation];
                var yRange = ySheet.Range[y.ShortLocation];

                if (xRange.Column < yRange.Column) return -1;
                if (xRange.Column > yRange.Column) return 1;
                //sort by row
                if (xRange.Row < yRange.Row) return -1;
            ...
            {
                foreach (var c in containers) c.ScenarioDataField.RegisterNextFocusField(c.ScenarioDataField);
        ...
        private void createContainer(Cell c, object cellData)
        {
            ...
            //create control
            var control = vsto.Controls.AddControl(container, currentWorksheet.Range[c.ShortLocation],
        ...
        public Scenario End()
        {
            ...
            foreach (var c in containers) c.ScenarioDataField.DataContext = null;

            //destroy controls
            foreach (Worksheet ws in workbook.Worksheets)
            {
                var vsto = Globals.Factory.GetVstoObject(ws);
                for (var i = vsto.Controls.Count - 1; i >= 0; i--)
                {
            ...
            lock (syncScenario)
            {
                ...
                if (resultScenario.Inputs.Count == 0 &&
                    ...
                    resultScenario.Conditions.Count == 0) return null;
